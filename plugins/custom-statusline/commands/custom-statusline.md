...
```
Question: "Which elements do you want in your status line?"
Header: "Statusline"
multiSelect: true
Options:
1. Label: "Current directory", Description: "Show working directory path"
2. Label: "Nerd Font icons", Description: "Show icons (requires Nerd Font installed)"
...
5. Label: "Session duration", Description: "Show time elapsed in session"
6. Label: "API usage (5h/7d)", Description: "Show Claude API usage percentages"
7. Label: "Context % till compact", Description: "Show context window usage"
```

## Step 2: Generate statusline.sh

Based on user selections, generate `~/.claude/statusline.sh` with this template.

The script reads a JSON config from `~/.claude/statusline-config.json` to determine which elements to show.
...
## Step 3: Write Config and Script

1. Write `~/.claude/statusline-config.json` with user's boolean selections
2. Write `~/.claude/statusline.sh` with the template above
3. Make it executable: `chmod +x ~/.claude/statusline.sh`
...
- Script saved to `~/.claude/statusline.sh`
